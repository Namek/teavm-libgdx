...
public class TeaVMGL20 implements GL20 {
    ...
    public void glTexImage2D(int target, int level, int internalformat, int width, int height, int border, int format,
        ...
        if(pixels == null){
            ...
            gl.texImage2D(target, level, internalformat, width, height, border, format, type,
                    copy((IntBuffer)pixels));
        } else if (pixels instanceof FloatBuffer) {
            gl.texImage2D(target, level, internalformat, width, height, border, format, type,
                    copy((FloatBuffer)pixels));
        } else if (pixels instanceof DoubleBuffer) {
            gl.texImage2D(target, level, internalformat, width, height, border, format, type,
                    copy((DoubleBuffer)pixels));
        } else {
    ...
    public void glGetFloatv(int pname, FloatBuffer params) {
        if (pname == GL20.GL_DEPTH_CLEAR_VALUE || pname == GL20.GL_LINE_WIDTH ||
                pname == GL20.GL_POLYGON_OFFSET_FACTOR || pname == GL20.GL_POLYGON_OFFSET_UNITS ||
