...
class Shot(model: Model, position: Vector3, var isInvaderShot: Boolean)// this.position.set(position);
    : ModelInstance(model, position) {
    var hasLeftField = false

    fun update(delta: Float) {
        if (isInvaderShot)
            transform.trn(0f, 0f, SHOT_VELOCITY * delta)
        else
            transform.trn(0f, 0f, -SHOT_VELOCITY * delta)
